...
async def create_coach(coach: CoachCreate, db: Session = Depends(get_db)):
    # Check if coach with same name already exists
    existing = coach_service.get_coach_by_name(db, coach.name)
    if existing:
...
async def get_coach(coach_id: int, db: Session = Depends(get_db)):
    coach = coach_service.get_coach(db, coach_id)
    if not coach:
...
async def update_coach(coach_id: int, coach_update: CoachUpdate, db: Session = Depends(get_db)):
    coach = coach_service.update_coach(db, coach_id, coach_update)
    if not coach:
        raise HTTPException(status_code=404, detail="Coach not found")
    return coach
